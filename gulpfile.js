...
gulp.task('dist', function () {
    ...
    gulp.src('./src/modules/style-guide/*.twig')
        .pipe(twig())
        .pipe(gulp.dest('./dist/modules/style-guide/')
        );
});
...
gulp.task('newfolder', function () {
    return gulp.src('*.*', { read: false })
        .pipe(gulp.dest('./src/modules/atoms/' + arg.name));

});

// New tiwg file
gulp.task('newtwig', function (callback) {
    fs.access('./src/modules/atoms/' + arg.name + '/' + arg.name + '.twig', (err) => {
        if (err) {
            fs.writeFile('./src/modules/atoms/' + arg.name + '/' + arg.name + '.twig', '', callback);
        }
        else {
...
gulp.task('newscss', function (callback) {
    fs.access('./src/modules/atoms/' + arg.name + '/' + arg.name + '.scss', (err) => {
        if (err) {
            fs.writeFile('./src/modules/atoms/' + arg.name + '/' + arg.name + '.scss', '', callback);
        }
        else {
...
gulp.task('newjs', function (callback) {
    fs.access('./src/modules/atoms/' + arg.name + '/' + arg.name + '.js', (err) => {
        if (err) {
            fs.writeFile('./src/modules/atoms/' + arg.name + '/' + arg.name + '.js', '', callback);
        }
        else {
